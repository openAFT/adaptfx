...
import interpol2D_OAR_tdist as intOAR
import interpol2D_tumor_tdist as inttumor
import numpy as np
import pandas as pd
        ...
        if self.var.get() == 0:
            ...
            try:
                ...
                if self.var_OAR.get() == 0:
                    ...
                    self.lbl_info[
                        "text"
                    ] = f"The optimal dose for fraction {len(sparing_factors)-1},  = {physical_dose[0]} \n dose delivered to tumor = {tumor_dose[0]}\n dose delivered OAR = {OAR_dose[0]}"
            except ValueError:
                self.lbl_info[
                    "text"
                ] = "please enter correct values\nsparing factors have to been inserted with space in between. No additional brackets needed."
