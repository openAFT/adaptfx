...
class GUIextended3D:
    def __init__(self, master):
        ...
        self.lbl_alpha = tk.Label(
            master=self.frm_probdis, text="shape of inverse-gamma distribution (alpha):"
        )
        ...
        self.lbl_beta = tk.Label(
            master=self.frm_probdis, text="scale of inverse-gamma distribution (beta):"
        )
        ...
        self.btn_BED_OAR = tk.Button(
            master=self.frm_form, text="?", command=self.info_funcs[6]
        )
    ...
    def info15(self):
        self.lbl_info[
            "text"
        ] = "Insert the accumulated tumor BED so far. (If fraction one, it is zero)."

    def info16(self):
        self.lbl_info[
            "text"
        ] = "Insert the accumulated OAR dose so far. (If fraction one, it is zero)."
                ...
                for i in range(1, number_of_fractions + 1):
                    for j in range(5):
                        if j == 0:
                            ...
                            label = tk.Label(
                                master=frame, text=f" {np.round(physical_doses[i-1],2)}"
                            )
                            label.pack()
                        elif j == 3:
                            frame = tk.Frame(master=self.lbl_output)
                            ...
                            label = tk.Label(
                                master=frame, text=f" {np.round(tumor_doses[i-1],2)}"
                            )
                            label.pack()
                        elif j == 4:
                            frame = tk.Frame(master=self.lbl_output)
